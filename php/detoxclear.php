...
$sql = "SELECT * FROM sanpham WHERE ten = 'Detox Clear' LIMIT 1";
...
$images = [];
if ($sp) {
    $tensp = strtolower(str_replace(' ', '', $sp['ten'])); // chuẩn hóa tên sản phẩm
    $img_sql = "SELECT imagePath FROM sanpham_images WHERE sanphamID = '$tensp'";
    $img_result = $conn->query($img_sql);
    if ($img_result && $img_result->num_rows > 0) {
        while ($row = $img_result->fetch_assoc()) {
            $images[] = '../images/' . $row['imagePath']; // đúng đường dẫn
        }
    }
}
...
function sendMessage() {
    ...
    <div class="bread-crumbs"> 
        <p>
            ...
            <a href="../thanhloc4.html">THANH LỌC CHUYÊN SÂU /</a>
            <span> DETOX CLEAR </span>
        </p>
    </div>
    <!-- SLIDE Detox 3 ngày-->
